 ...
 *
 * Disassembly of ../Results/DSDT.aml, Sun Nov 15 02:51:39 2020
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0001322C (78380)
 *     Revision         0x02
 *     Checksum         0x7E
 *     OEM ID           "DELL  "
 *     OEM Table ID     "CBX3   "
...
DefinitionBlock ("", "DSDT", 2, "DELL  ", "CBX3   ", 0x01072009)
{
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0xCBD04000, 0x035B)
    ...
    OperationRegion (EXBU, SystemMemory, 0xCBD09000, 0x0011)
    ...
    OperationRegion (OGNV, SystemMemory, 0xCBEDD000, 0x0001)
    ...
    Scope (_SB)
    {
        Device (PCI0)
        {
            ...
            Device (LPCB)
            {
                ...
                OperationRegion (CPSB, SystemMemory, 0xCB388E98, 0x10)
    ...
    If ((SMD1 != 0x02))
    {
        Scope (_SB.PCI0.I2C0)
        {
            ...
            Method (_STA, 0, NotSerialized)  // _STA: Status
            {
                ...
                If (((CLEG == 0x1E) && (OSYS == 0x07DD)))
                {
                    Return (Zero)
                }

                If (((CLEG == 0x1E) && (OSYS == 0x07DC)))
                {
                    Return (Zero)
                }
    ...
    Method (NEVT, 0, NotSerialized)
    {
        ...
        If ((Local1 & 0x0100))
        {
            Local3 = ECBT (One, 0x08)
            If (Local3)
            {
                If (CondRefOf (\_PR.CFGD))
                {
                    If ((!(\_PR.CFGD & One) || (\_PR.CFGD & 0x0100)))
                    {
                        GENS (0x0A, One, Zero)
                    }
                }
    ...
    {
        Mutex (MSMI, 0x01)
        OperationRegion (CNVS, SystemMemory, 0xCBD07018, 0x00000800)
